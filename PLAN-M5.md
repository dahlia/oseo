...
settlement functions and leaves an empty input pending. An abrupt resolve call,
`then` getter, or `then` call closes an unfinished iterator before rejection
while preserving the original throw completion; an abrupt iterator step or
value read and final capability resolution retain the iterator's completed
...
192 paths under the node's two inventory roots, 190 are reviewed: 172 pass and
18 retain explicit prerequisite boundaries for the unhandled-rejection host
