...
function throwIndex(testCase: ArraySpreadCase): number {
  return testCase.throwRaw % (testCase.values.length + 1);
}

function iterableSource(testCase: ArraySpreadCase): string {
  ...
  const captured =
    testCase.mode === "captured-next"
      ? "this.next = function () { return { value: -1, done: true }; };"
      : "";
  const throwing =
...
function printCase(testCase: ArraySpreadCase): string {
  const successful =
    testCase.mode !== "non-iterable" && testCase.mode !== "throw-next";
  const resultLength =
    testCase.values.length + testCase.secondValues.length + 3;
  const observations = successful
    ? Array.from({ length: resultLength }, (_, index) => `result[${index}]`)
    : [];
  return `
let steps = 0;
let closes = 0;
...
try {
  ...
  console.log("array", result.length, ${observations.join(", ")});
} catch (error) {
...
function expectedOutput(testCase: ArraySpreadCase): string {
  if (testCase.mode === "non-iterable") {
    return "error TypeError\nstate 0 0 prefix,spread\n";
  }
  if (testCase.mode === "throw-next") {
    return (
      "error RangeError\n" +
      `state ${throwIndex(testCase) + 1} 0 prefix,spread\n`
    );
  }
  const values = [
    testCase.prefix,
  ...
  return (
    `array ${values.length} ${values.map(String).join(" ")}\n` +
    `state ${steps} 0 prefix,spread,second,suffix\n`
  );
}
...
test(
  "generated array spread cases match the M5 accumulation model",
  {
    skip: nativeTarget == null ? "requires a supported native host" : false,
  },
  async () => {
    await assertAsyncProperty(
      "array spread preserves generated accumulation observations",
      fc.asyncProperty(caseArbitrary, async (testCase) => {
        ...
        for (const specialization of ["disabled", "enabled"] as const) {
          const compiled = compileSource(
            babelFrontend,
            { source, sourceId: "generated-m5-array-spread.js" },
            { specialization },
          );
      ...
      }),
      {
        ...
        domain:
          "bounded multi-spread arrays, custom iterables, captured next, " +
          "and abrupt steps",
        numRuns: 10,
