...
// The upstream TypedArray harness, unchanged in behavior. The repository
// formatter owns every JavaScript file here, so spacing differs from
// upstream while every operation is identical. Upstream reads isPrimitive
// from assert.js, which the reviewed base harness replaces, so the same
// predicate is defined here. Upstream also probes `typeof Float16Array`,
// a post-2025 global that this profile rejects as an unbound standard
// name at compile time; the two probes are removed with the outcome they
// always have on an ECMAScript 2025 host. The admitted ArrayBuffer surface
// has resize and no transferToImmutable, so the constructor-argument
// factories are exactly the ones upstream selects for such a host.
...
// Float16Array is a newer feature. The ECMAScript 2025 profile has no such
// global, so the upstream typeof probe always leaves this list unchanged.
 ...
 */
var allTypedArrayConstructors = typedArrayConstructors.concat(
...
if (ArrayBuffer.prototype.resize) {
  ...
  makeResizableArrayBuffer = function makeResizableArrayBuffer(
    TA,
    primitiveOrIterable,
  ) {
    if (isPrimitive(primitiveOrIterable)) {
    ...
    var resizable = new ArrayBuffer(byteLength, {
      maxByteLength: byteLength * 2,
    });
    return copyIntoArrayBuffer(resizable, fixed);
  };

  makeGrownArrayBuffer = function makeGrownArrayBuffer(
    TA,
    primitiveOrIterable,
  ) {
    if (isPrimitive(primitiveOrIterable)) {
    ...
    var fixed = makeArrayBuffer(TA, primitiveOrIterable);
    var byteLength = fixed.byteLength;
    var grown = new ArrayBuffer(Math.floor(byteLength / 2), {
  ...
  makeShrunkArrayBuffer = function makeShrunkArrayBuffer(
    TA,
    primitiveOrIterable,
  ) {
    if (isPrimitive(primitiveOrIterable)) {
    ...
    var fixed = makeArrayBuffer(TA, primitiveOrIterable);
    var byteLength = fixed.byteLength;
    var shrunk = new ArrayBuffer(byteLength * 2, {
      maxByteLength: byteLength * 2,
    });
...
if (ArrayBuffer.prototype.transferToImmutable) {
  makeImmutableArrayBuffer = function makeImmutableArrayBuffer(
    TA,
    primitiveOrIterable,
  ) {
    if (isPrimitive(primitiveOrIterable)) {
...
if (makeShrunkArrayBuffer)
  typedArrayCtorArgFactories.push(makeShrunkArrayBuffer);
  ...
  for (var i = 0; i < features.length; ++i) {
    switch (features[i]) {
      case "passthrough":
        ...
        break;
      case "arraylike":
        if (argFactory === makeArray || argFactory === makeArrayLike)
          return true;
        break;
      case "iterable":
 ...
 *   constructor argument factory functions, after an initial selection
 */
function testWithNonAtomicsFriendlyTypedArrayConstructors(
  f,
  includeArgFactories,
  excludeArgFactories,
) {
 ...
 *   constructor argument factory functions, after an initial selection
 */
function testWithAtomicsFriendlyTypedArrayConstructors(
  f,
  includeArgFactories,
  excludeArgFactories,
) {
