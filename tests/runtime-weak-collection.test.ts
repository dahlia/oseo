...
const fixture = join(root, "tests/fixtures/runtime-weak-collection.c");
...
test(
  "runs the weak collection runtime contract on the host-native target",
  {
    skip: zigNativeTarget == null ? "requires a supported native host" : false,
  },
  async () => {
    const directory = await mkdtemp(join(tmpdir(), "oseo-weak-collection-"));
    try {
      const executable = join(directory, "runtime-weak-collection");
      ...
      run("zig", [
        ...
        ...sources,
        "-o",
        join(directory, "runtime-weak-collection-aarch64"),
      ]);
    } finally {
