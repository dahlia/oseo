...
--------
 ...
 -  Context propagation through every admitted asynchronous form depends
    on two recorded M5 gaps: the restricted `await` positions in ordinary
    asynchronous bodies, and the drain-based suspension of `for await`, of
    `yield*` in asynchronous generators, and of top-level await. A
    drain-based form runs unrelated jobs before returning to its caller,
    so a dispatcher-level restore would leave the suspending job's
    snapshot installed for those jobs. These forms move to
...
------------
 ...
 -  The restricted-`await` and drain-based suspension gaps gain a further
