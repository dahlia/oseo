...
export function statement(
  ...
  if (value.type === "ForInStatement") {
    ...
    if (left.type === "VariableDeclaration") {
      ...
      if (rejectedThisBindingName(context, identifier, name)) return undefined;
      if (name == null) {
        // A recursive binding pattern reuses the declaration pattern the
        // profile already owns. A reserved word used as a binding name is an
        // early error rather than a profile boundary, so the whole head is
        // checked first, exactly as the iterate head's conversion does.
      ...
      if (name != null) {
        ...
        // Reuse the recursive destructuring assignment pattern the profile
        // already owns for both object and array heads.
