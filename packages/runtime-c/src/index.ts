import type { RuntimeInput, RuntimeInputProvider } from "@oseo/compiler";

const runtimeInput: RuntimeInput = {
  abiVersion: "m5-125",
  assets: [
    {
      kind: "header",
      name: "oseo_runtime.h",
      url: new URL("../native/oseo_runtime.h", import.meta.url),
    },
    {
      kind: "header",
      name: "runtime_internal.h",
      url: new URL("../native/runtime_internal.h", import.meta.url),
    },
    {
      kind: "header",
      name: "runtime_unicode_tables.h",
      url: new URL("../native/runtime_unicode_tables.h", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_core.c",
      url: new URL("../native/runtime_core.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_memory.c",
      url: new URL("../native/runtime_memory.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_binding.c",
      url: new URL("../native/runtime_binding.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_string.c",
      url: new URL("../native/runtime_string.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_string_match.c",
      url: new URL("../native/runtime_string_match.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_object.c",
      url: new URL("../native/runtime_object.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_property.c",
      url: new URL("../native/runtime_property.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_descriptor.c",
      url: new URL("../native/runtime_descriptor.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_array.c",
      url: new URL("../native/runtime_array.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_object_builtin.c",
      url: new URL("../native/runtime_object_builtin.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_number.c",
      url: new URL("../native/runtime_number.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_boolean.c",
      url: new URL("../native/runtime_boolean.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_array_buffer.c",
      url: new URL("../native/runtime_array_buffer.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_set.c",
      url: new URL("../native/runtime_set.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_typed_array.c",
      url: new URL("../native/runtime_typed_array.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_weak_collection.c",
      url: new URL("../native/runtime_weak_collection.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_arguments.c",
      url: new URL("../native/runtime_arguments.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_enumeration.c",
      url: new URL("../native/runtime_enumeration.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_function.c",
      url: new URL("../native/runtime_function.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_error.c",
      url: new URL("../native/runtime_error.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_symbol.c",
      url: new URL("../native/runtime_symbol.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_iterator.c",
      url: new URL("../native/runtime_iterator.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_generator.c",
      url: new URL("../native/runtime_generator.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_async_generator.c",
      url: new URL("../native/runtime_async_generator.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_bigint.c",
      url: new URL("../native/runtime_bigint.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_primitive.c",
      url: new URL("../native/runtime_primitive.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_promise.c",
      url: new URL("../native/runtime_promise.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_event_loop.c",
      url: new URL("../native/runtime_event_loop.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_clock.c",
      url: new URL("../native/runtime_clock.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_clock_posix.c",
      url: new URL("../native/runtime_clock_posix.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_map.c",
      url: new URL("../native/runtime_map.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_bigint_object.c",
      url: new URL("../native/runtime_bigint_object.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_data_view.c",
      url: new URL("../native/runtime_data_view.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_date.c",
      url: new URL("../native/runtime_date.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_regexp.c",
      url: new URL("../native/runtime_regexp.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_regexp_matcher.c",
      url: new URL("../native/runtime_regexp_matcher.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_regexp_symbol.c",
      url: new URL("../native/runtime_regexp_symbol.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_math.c",
      url: new URL("../native/runtime_math.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_uri.c",
      url: new URL("../native/runtime_uri.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_reflect.c",
      url: new URL("../native/runtime_reflect.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_proxy.c",
      url: new URL("../native/runtime_proxy.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_json.c",
      url: new URL("../native/runtime_json.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_atomics.c",
      url: new URL("../native/runtime_atomics.c", import.meta.url),
    },
    {
      kind: "source",
      name: "runtime_agent.c",
      url: new URL("../native/runtime_agent.c", import.meta.url),
    },
  ],
};

/** Reviewed C runtime inputs kept separate from generated JavaScript. */
export const cRuntimeProvider: RuntimeInputProvider = {
  getRuntimeInput(): RuntimeInput {
    return runtimeInput;
  },
};
