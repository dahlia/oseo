...
static OseoResult global_this_object(OseoContext *context) {
    ...
    if (result.status == OSEO_STATUS_NORMAL) {
        result = oseo_internal_install_boolean_global(
            context,
            frame.slots[0]
        );
    }
    ...
    if (result.status == OSEO_STATUS_NORMAL) {
        result = oseo_internal_install_set_global(
            context,
            frame.slots[0]
        );
    }
    if (result.status == OSEO_STATUS_NORMAL) {
        result = oseo_internal_install_weak_collection_globals(
            context,
            frame.slots[0]
        );
    }
    ...
    if (result.status == OSEO_STATUS_NORMAL) {
        result = oseo_internal_install_data_view_global(
            context,
            frame.slots[0]
        );
    }
    if (result.status == OSEO_STATUS_NORMAL) {
        result = oseo_internal_install_date_global(
            context,
            frame.slots[0]
        );
    }
    if (result.status == OSEO_STATUS_NORMAL) {
        result = oseo_internal_install_regexp_global(
            context,
            frame.slots[0]
        );
    }
    ...
    if (result.status == OSEO_STATUS_NORMAL) {
        result = oseo_internal_install_uri_global(
            context,
            frame.slots[0]
        );
    }
    if (result.status == OSEO_STATUS_NORMAL) {
        result = oseo_internal_install_global_numeric_functions(
            context,
            frame.slots[0]
        );
    }
    if (result.status == OSEO_STATUS_NORMAL) {
        result = oseo_internal_install_reflect_global(
            context,
            frame.slots[0]
        );
    }
    if (result.status == OSEO_STATUS_NORMAL) {
        result = oseo_internal_install_proxy_global(
            context,
            frame.slots[0]
        );
    }
    if (result.status == OSEO_STATUS_NORMAL) {
        result = oseo_internal_install_json_global(
            context,
            frame.slots[0]
        );
    }
    if (result.status == OSEO_STATUS_NORMAL) {
        result = oseo_internal_install_shared_array_buffer_global(
            context,
            frame.slots[0]
        );
    }
    if (result.status == OSEO_STATUS_NORMAL) {
        result = oseo_internal_install_atomics_global(
            context,
            frame.slots[0]
        );
    }
