...
OseoResult oseo_internal_array_builtin_dispatch(
    ...
    if (code_id == OSEO_ARRAY_IS_ARRAY_CODE_ID) {
        OseoValue value = argument_count == 0u
            ? oseo_undefined()
            : arguments[0];
        return oseo_internal_is_array(context, value);
    }
...
static OseoResult array_is_concat_spreadable(
    ...
    *spreadable = false;
    ...
    if (result.status == OSEO_STATUS_NORMAL &&
        ...
        result = oseo_internal_is_array(context, slots[0]);
        if (result.status == OSEO_STATUS_NORMAL) {
            *spreadable = oseo_to_boolean(result.value);
        }
...
static OseoResult flatten_into_array(
    ...
    for (double source_index = 0.0;
        ...
        bool flatten = false;
        if (depth > 0.0) {
            result = oseo_internal_is_array(context, slots[5]);
            if (result.status == OSEO_STATUS_NORMAL) {
                flatten = oseo_to_boolean(result.value);
            }
        }
        if (result.status != OSEO_STATUS_NORMAL) break;
        if (flatten) {
...
static OseoResult array_sorting(
    ...
    if (tag_of(comparator) != OSEO_TAG_UNDEFINED && !is_callable(comparator)) {
        return type_error(
            context,
            copy
...
static OseoResult array_species_create(
    ...
    result = oseo_internal_is_array(context, frame.slots[0]);
    if (result.status != OSEO_STATUS_NORMAL) {
        oseo_roots_release(context, &frame);
        return result;
    }
