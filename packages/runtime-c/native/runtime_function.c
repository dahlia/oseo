...
static const OseoBuiltinDispatchRange builtin_dispatch_ranges[] = {
    ...
    {OSEO_ARRAY_BUFFER_CODE_ID_RANGE_FIRST,
     OSEO_ARRAY_BUFFER_CODE_ID_RANGE_LAST,
     oseo_internal_array_buffer_builtin_dispatch},
    ...
    {OSEO_BIGINT_CODE_ID_RANGE_FIRST,
     OSEO_BIGINT_CODE_ID_RANGE_LAST,
     oseo_internal_bigint_builtin_dispatch},
    {OSEO_DATA_VIEW_CODE_ID_RANGE_FIRST,
     OSEO_DATA_VIEW_CODE_ID_RANGE_LAST,
     oseo_internal_data_view_builtin_dispatch},
    ...
    {OSEO_DATE_CODE_ID_RANGE_FIRST,
     OSEO_DATE_CODE_ID_RANGE_LAST,
     oseo_internal_date_builtin_dispatch},
    {OSEO_JSON_CODE_ID_RANGE_FIRST,
     OSEO_JSON_CODE_ID_RANGE_LAST,
     oseo_internal_json_builtin_dispatch},
    {OSEO_SET_CODE_ID_RANGE_FIRST,
     OSEO_SET_CODE_ID_RANGE_LAST,
     oseo_internal_set_builtin_dispatch},
    {OSEO_TYPED_ARRAY_CODE_ID_RANGE_FIRST,
     OSEO_TYPED_ARRAY_CODE_ID_RANGE_LAST,
     oseo_internal_typed_array_builtin_dispatch},
    {OSEO_ATOMICS_CODE_ID_RANGE_FIRST,
     OSEO_ATOMICS_CODE_ID_RANGE_LAST,
     oseo_internal_atomics_builtin_dispatch},
...
OseoResult oseo_intrinsic(OseoContext *context, OseoIntrinsic intrinsic) {
    ...
    if (intrinsic == OSEO_INTRINSIC_OBJECT_PROTOTYPE ||
    ...
    } else if (intrinsic == OSEO_INTRINSIC_BIGINT_PROTOTYPE ||
    ...
    } else if (intrinsic >= OSEO_INTRINSIC_DATA_VIEW_PROTOTYPE &&
               intrinsic <= OSEO_INTRINSIC_DATA_VIEW_SET_BIG_UINT64) {
        materialized = oseo_internal_data_view_intrinsic(context);
    } else if (intrinsic == OSEO_INTRINSIC_DATE_PROTOTYPE ||
               intrinsic == OSEO_INTRINSIC_DATE) {
        materialized = oseo_internal_date_intrinsic(context);
    ...
    } else if (intrinsic == OSEO_INTRINSIC_JSON ||
               intrinsic == OSEO_INTRINSIC_JSON_PARSE) {
        materialized = oseo_internal_json_intrinsic(context);
    } else if (intrinsic >= OSEO_INTRINSIC_SET_PROTOTYPE &&
               intrinsic <= OSEO_INTRINSIC_SET_SPECIES) {
        materialized = oseo_internal_set_intrinsic(context);
    } else if (intrinsic >= OSEO_INTRINSIC_TYPED_ARRAY_PROTOTYPE &&
               intrinsic <= OSEO_INTRINSIC_BIGUINT64_ARRAY) {
        materialized = oseo_internal_typed_array_intrinsic(context);
    } else if (
        intrinsic >= OSEO_INTRINSIC_SHARED_ARRAY_BUFFER_PROTOTYPE &&
        intrinsic <= OSEO_INTRINSIC_SHARED_ARRAY_BUFFER_SPECIES
    ) {
        materialized = oseo_internal_shared_array_buffer_intrinsic(context);
    } else if (intrinsic == OSEO_INTRINSIC_ATOMICS) {
        materialized = oseo_internal_atomics_intrinsic(context);
    } else if (intrinsic >= OSEO_INTRINSIC_WEAK_MAP_PROTOTYPE &&
               intrinsic <= OSEO_INTRINSIC_FINALIZATION_REGISTRY_UNREGISTER) {
        materialized = oseo_internal_weak_collection_intrinsic(context);
        ...
        materialized = oseo_internal_iterator_helper_prototype(context);
    ...
    } else if (intrinsic == OSEO_INTRINSIC_ARRAY_KEYS) {
        materialized = oseo_internal_iterator_method(
            context,
            OSEO_ARRAY_KEYS_CODE_ID
        );
    } else if (intrinsic == OSEO_INTRINSIC_ARRAY_ENTRIES) {
        materialized = oseo_internal_iterator_method(
            context,
            OSEO_ARRAY_ENTRIES_CODE_ID
        );
    } else if (intrinsic == OSEO_INTRINSIC_STRING_PROTOTYPE_ITERATOR) {
        materialized = oseo_internal_iterator_method(
            context,
            OSEO_STRING_PROTOTYPE_ITERATOR_CODE_ID
        );
    } else if (intrinsic == OSEO_INTRINSIC_STRING_ITERATOR_PROTOTYPE ||
               intrinsic == OSEO_INTRINSIC_STRING_ITERATOR_NEXT) {
        materialized = oseo_internal_string_iterator_prototype(context);
...
OseoResult oseo_function_create(
    ...
    function->ordinary.iterator_kind = OSEO_ITERATOR_NONE;
    function->ordinary.iterator_target = oseo_undefined();
