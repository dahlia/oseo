...
converted key, and property read before evaluating its right operand. Binary
forms reuse the corresponding MIR operator before a checked write. Logical
forms lower through explicit branches, so the skipped path performs no right
evaluation or write. Await inside a compound assignment remains unsupported
until module continuation extraction can preserve that retained current value.
